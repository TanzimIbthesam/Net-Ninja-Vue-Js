...
<div id="add-blog">
  ...
  <form v-if="!submitted">
    ...
    <input type="text" v-model.lazy="blog.title" placeholder="Add Title" required><br>
     <label for="">Add Content</label><br>
  ...
  <option v-for="(author) in blog.authors" v-bind:key="author">{{author}}</option>
...
export default{
 ...
 methods:{
        post:function(){
            this.$http.post('https://vue-blog-a9f15.firebaseio.com/posts.json',this.blog).then(function(data){
                console.log(data);
                this.submitted = true;
            });
    }
